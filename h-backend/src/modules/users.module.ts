...
export class UsersModule implements NestModule {
    configure(consumer: MiddlewareConsumer) {
        ...
        consumer.apply(auth, verifyModifyInfoParamsNotNull, EncryptPassword).forRoutes({ path: "", method: RequestMethod.PUT });
        consumer.apply(auth, verifyLoginAndLogoutParamsNotNull, EncryptPassword).forRoutes({ path: "", method: RequestMethod.DELETE })
    }
}